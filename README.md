...
| `AI_MODEL` | 任意 | 使うClaudeモデル名。未設定なら `claude-3-5-haiku-20241022`（固定ID推奨） |
...
- **既定モデル: `claude-3-5-haiku-20241022`（固定ID）**
  - `claude-3-5-haiku-latest` のような latest 系は将来中身が変わる可能性があるため、本番・定時実行では固定IDを推奨する
  - `AI_MODEL` 環境変数で上書き可能だが、通常運用では固定IDを使う
...
- AI モデルのデフォルトは `claude-3-5-haiku-20241022`（固定ID、本番運用向け）
- 取得対象期間は直近 3 日
